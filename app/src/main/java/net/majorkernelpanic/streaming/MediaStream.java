		...
		if (mStreaming) {
			try {
				if (mMode==MODE_MEDIARECORDER_API) {
					...
					// let's see if moov atom is added
					// -----------------------------------------untested----------------------------------------------
					//int bufferIndex = mMediaCodec.dequeueInputBuffer(500000);
					//mMediaCodec.queueInputBuffer(bufferIndex, 0, 0, System.nanoTime()/1000, MediaCodec.BUFFER_FLAG_END_OF_STREAM);

					//mPacketizer.setInputStream(new MediaCodecInputStream(mMediaCodec));
					//mPacketizer.start();

					// do we need to wait for a while?
					// -----------------------------------------untested----------------------------------------------
					...
					mMediaCodec.release();
					mMediaCodec = null;
/*					File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/movie.mp4");
					SequenceEncoder encoder = SequenceEncoder.createSequenceEncoder(file, 25);
					AndroidSequenceEncoder encoder1 = new AndroidSequenceEncoder((SeekableByteChannel)file, Rational.ONE);

					for (final File child : new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/tensorflow/movie/").listFiles()){
						Picture frame = Picture.create(640, 480, ColorSpace.YUV420);
						// convert bitmaps to pictures, then save them
						Bitmap image = BitmapFactory.decodeFile(child.getAbsolutePath());
						BufferedInputStream
						//encoder1.encodeNativeFrame(image.);
					}*/

...
public class SessionBuilder {
	...
	private static int mCamera = CameraInfo.CAMERA_FACING_BACK;
	private static Camera realCamera;
		...
		if (sInstance == null) {
			synchronized (SessionBuilder.class) {
				if (sInstance == null) {
					...
					//realCamera = Camera.open(mCamera);
				}
			}
		}
		return sInstance;
	...
	public Session build() {
		...
		case VIDEO_H264:
            ...
            H264Stream stream = new H264Stream(realCamera);
			if (mContext!=null) 
				stream.setPreferences(PreferenceManager.getDefaultSharedPreferences(mContext));
	...
	public SessionBuilder setCamera(Camera camera) {
		realCamera = camera;
		return this;
	}
	...
	/* Return a cameraObject*/
	public Camera getCameraObject(){return realCamera;}
